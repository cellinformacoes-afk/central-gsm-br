...
async function creditUser() {
  ...
  if (pendingTxs && pendingTxs.length > 0) {
    for (const tx of pendingTxs) {
      await supabase
        .from('transactions')
        .update({ status: 'success', amount: amountToAdd, description: 'Creditado manualmente via script' })
        .eq('id', tx.id);
    ...
    await supabase
      ...
      .insert({
        ...
        status: 'success',
        type: 'pix',
        description: 'Creditado manualmente via script (74 reais)'
      });
