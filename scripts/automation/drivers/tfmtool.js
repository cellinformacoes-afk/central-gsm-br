...
async function resetPassword(page, email, oldPassword, newPassword, fallbackPassword) {
  try {
    ...
    await page.goto('https://tfmtool.com/user', { waitUntil: 'load', timeout: 60000 }).catch(() => {});
    ...
    if (!(await isLogged())) {
      ...
      await page.goto('https://tfmtool.com/login', { waitUntil: 'load' });
      await handleCloudflare(page);
    ...
    if (await isLogged()) {
        ...
        await page.goto('https://tfmtool.com/user/change-password', { waitUntil: 'load' }).catch(() => {});
    ...
    console.error(`Erro no driver TFM: ${err.message}`);
    return false;
  }
}
