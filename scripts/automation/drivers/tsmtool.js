...
async function resetPassword(page, email, oldPassword, newPassword, fallbackPassword) {
  try {
    ...
    await page.goto('https://tsm-tool.com/login', { waitUntil: 'load', timeout: 60000 }).catch(() => {});
    ...
    if (!(await isLogged())) {
      ...
      const emailSel = 'input[placeholder*="Username"], input[name="username"], input[type="email"]';
    ...
    if (await isLogged()) {
        ...
        await page.goto('https://tsm-tool.com/?act=UserUpdate', { waitUntil: 'load' }).catch(() => {});
    ...
    console.error(`Erro no driver TSM: ${err.message}`);
    return false;
  }
}
