...
/**
 * Driver para UnlockTool.net
 **/
async function resetPassword(page, email, oldPassword, newPassword, fallbackPassword) {
  try {
    console.log(`\n[UNLOCK TOOL] Iniciando para ${email}...`);
    
    // Passo 2: Limpando tradução
    ...
    await page.goto('https://unlocktool.net/post-in/', { waitUntil: 'load', timeout: 60000 }).catch(() => {});
    ...
    if (!(await isLogged())) {
        ...
        await page.goto('https://unlocktool.net/login/', { waitUntil: 'load' }).catch(() => {});
        await handleCloudflare(page);

        console.log(`... Passo 5: Preenchendo campos`);
        const emailSel = 'input#id_username, input[name="username"], input[name="log"]';
    ...
    if (await isLogged()) {
        ...
        await page.goto('https://unlocktool.net/account/password/', { waitUntil: 'load' }).catch(() => {});
        ...
        if (passFields.length >= 2) {
            ...
            await page.click('button:has-text("Save"), .btn-success');
    ...
    console.error(`Erro no driver UnlockTool: ${err.message}`);
    return false;
  }
}
